...
import CreateEditFrontend from '#/pages/create-frontend/CreateEditFrontend';
import { useGetFrontendQuery, useEditFrontendMutation, EditFrontendPayload } from '#/lib/store/slice/apiSlice';
...
const EditFrontendPage: React.FC = () => {
  ...
  const [editFrontend, { isLoading: isEditLoading }] = useEditFrontendMutation();

  // Fetch the specific frontend data
  const { data: frontend, isLoading: isFrontendLoading, isError: isFrontendError } = useGetFrontendQuery(null);

  const currentFrontend = frontend?.find((f: Frontend) => f.Identifier === frontendId);

  // Handle form submission for editing
  const handleEditSubmit = async (values: Omit<EditFrontendPayload, 'Identifier'>) => {
    try {
  ...
  if (isFrontendLoading) {
    return (
      <PageContainer>
        <OllamaFlowFlex justify="center" align="center" style={{ height: '400px' }}>
  ...
  if (isFrontendError || !currentFrontend) {
    return (
      <PageContainer>
        <OllamaFlowFlex vertical gap={20} align="center" style={{ height: '400px' }}>
    ...
    <PageContainer
      pageTitle={
        <OllamaFlowFlex align="center" gap={10}>
          <OllamaFlowButton type="text" icon={<ArrowLeftOutlined />} onClick={handleBack} style={{ padding: 0 }} />
