...
import { useGetBackendQuery, useDeleteBackendMutation } from '#/lib/store/slice/apiSlice';
...
const BackendsListingPage: React.FC = () => {
  ...
  const { data: backends = [], isLoading, isFetching, isError, refetch } = useGetBackendQuery(null);
  const isBackendLoading = isLoading || isFetching;

  const [deleteBackend, { isLoading: isDeleteLoading }] = useDeleteBackendMutation();
    ...
    >
      <OllamaFlowTable
        ...
        pagination={{
          ...
          showTotal: (total, range) => `${range[0]}-${range[1]} of ${total} backends`,
        }}
        size="middle"
      />
      ...
      >
        {deletingBackend && (
          <p>
            Are you sure you want to delete the backend <strong>&ldquo;{deletingBackend.Name}&rdquo;</strong> (
